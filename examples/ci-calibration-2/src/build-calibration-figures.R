...
plot_df <- input_list$estimates |> filter(variable == "prevalence") |> select(-variable) |> dcast(replicate + from_aggregated ~ quantile)
...
prev_fig_1 <- ggplot(data = plot_df) +
  geom_linerange(mapping = aes(x = sorted_order, ymin = q1, ymax = q5), 
                 colour = green_hex_colour) +
  geom_point(mapping = aes(x = sorted_order, y = q3), 
             colour = green_hex_colour) +
  geom_hline(data = data.frame(yi = c(0, true_r_naught), variable = c("prevalence","r_naught")), 
             mapping = aes(yintercept = yi), 
             linetype = "dashed") +
  facet_grid(variable~., 
             scale = "free_y", 
             labeller = labeller(variable = facet_labels)) +
  labs(x = "Replicate", y = NULL, colour = NULL) +
  theme_classic() +
  theme(axis.text.x = element_blank(),
        strip.background = element_blank(),
        legend.position = "null")
...
if (!interactive()) {
  ...
  args <- list(input = "out/foo-summary.json",
