...
Rscript src/summarise-results.R $NUM_SEEDS $VIS_JSON
...
Making the figures is a two step process: first, the results are summarised by
the =summarise-results.R= script and stored in a JSON file; second, the
