...
simulatedObservations bdscodConfig = do
  simEvents <- liftIO $ SimUtil.simulation False bdscodConfig SimBDSCOD.allEvents -- simulate the epidemic
  liftIO $ print simEvents
  simEventsCsv <- asks simulatedEventsOutputCsv
  liftIO $ L.writeFile simEventsCsv (Csv.encode simEvents) -- record a list of all events
  ...
  case maybeObs of
    (Just obs) -> do (reconNewickTxt,reconNewickCsv) <- asks reconstructedTreeOutputFiles
                     -- record the full transmission tree in newick
                     ...
                     liftIO $ Prelude.writeFile obsCsv $ intercalate "\n" (map show obs)
                     return obs
